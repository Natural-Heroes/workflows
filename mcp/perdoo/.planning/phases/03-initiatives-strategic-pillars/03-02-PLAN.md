...
<task type="auto">
  ...
  <files>
    src/scripts/introspect-goals.ts
    src/services/perdoo/operations/strategic-pillars.ts
    src/services/perdoo/types.ts
    src/services/perdoo/client.ts
  </files>
  <action>
    **Step 1: Create and run introspection script for Goal schema.**

    Create `src/scripts/introspect-goals.ts` that uses the existing PerdooClient to run targeted __type queries. The script should:
    - Import createPerdooClient from the services layer
    - Run each introspection query sequentially
    - Log results to stderr as JSON for debugging
    - The PERDOO_API_TOKEN env var must be set
    - Execute with: `npx tsx src/scripts/introspect-goals.ts`
    ...
    **Step 4: Add client methods to `src/services/perdoo/client.ts`:**
    ...
    - Add section: `// Strategic Pillar Operations` (after the Initiative Operations section added in Plan 03-01, before Introspection)
