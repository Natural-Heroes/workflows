...
export function createServiceUnavailableError(): McpToolError {
  return new McpToolError({
    userMessage: 'The Inventory Planner service is temporarily unavailable. Try again later.',
    isRetryable: true,
...
export function createAuthenticationError(): McpToolError {
  return new McpToolError({
    ...
    isRetryable: false,
    suggestedAction: 'Check that INVENTORY_PLANNER_API_KEY and INVENTORY_PLANNER_ACCOUNT_ID are correct.',
 ...
 *
 * Used when the Inventory Planner API rejects a request with HTTP 400.
 *
 * @param apiMessage - Error message from the API response
 * @returns McpToolError with the API's error message
 */
export function createApiValidationError(apiMessage: string): McpToolError {
  return new McpToolError({
    userMessage: `Inventory Planner API rejected the request: ${apiMessage}`,
    isRetryable: false,
