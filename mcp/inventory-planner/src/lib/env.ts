 ...
 *
 * Required:
 * - INVENTORY_PLANNER_API_KEY: API key from Inventory Planner account
 * - INVENTORY_PLANNER_ACCOUNT_ID: Account ID from Inventory Planner
 *
...
const envSchema = z.object({
  INVENTORY_PLANNER_API_KEY: z
    .string({ required_error: 'INVENTORY_PLANNER_API_KEY is required' })
    .min(1, 'INVENTORY_PLANNER_API_KEY cannot be empty'),

  INVENTORY_PLANNER_ACCOUNT_ID: z
    .string({ required_error: 'INVENTORY_PLANNER_ACCOUNT_ID is required' })
    .min(1, 'INVENTORY_PLANNER_ACCOUNT_ID cannot be empty'),
