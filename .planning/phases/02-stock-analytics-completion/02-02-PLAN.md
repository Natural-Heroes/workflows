...
**Implementation pattern:** Call client.getVariants with a fields filter (warehouse_id, warehouse_name for warehouses; vendor_id, vendor_name, vendors for vendors) to fetch variants with minimal data. Extract unique warehouses/vendors using a Map keyed by ID to deduplicate. Return formatted response with summary count and array of {id, name} objects.
